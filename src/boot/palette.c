...
static void init_palettes(EfiPalettes *palettes) {
        memcpy(palettes, &efi_palettes_default, sizeof (EfiPalettes));
}

static void parse_palette(EfiPaletteType type, char *value, EfiPalette *ret_palette) {
        size_t pos = 0;
        ...
        if (fg_string != NULL && (fg_color < -1 || bg_string == NULL))
                ...
                log_error("Error parsing 'efi-palette-%s' config option, ignoring index %zu: %s",
                          efi_palette_type_to_string(type), i * 2 + 1, fg_string);
                ...
                log_error("Error parsing 'efi-palette-%s' config option, ignoring index %zu: %s",
                          efi_palette_type_to_string(type), i * 2 + 1, bg_string);
