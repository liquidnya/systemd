...
#define CONFIG_PALETTE(config, type) ((config)->efi_palletes[(EFI_PALETTE_##type)])
...
static void print_status(Config *config, char16_t *loaded_image_path) {
        ...
        for (size_t i = 0; i < _EFI_PALETTE_TYPE_MAX; i++) {
                const char *name = efi_color_to_string(EFI_TEXT_ATTR_FG(palette.text));
                for (size_t j = strlen(name); j < 18; j++)
                        printf(" ");
                printf("efi-palette-%s: ", name);
                for (size_t j = 0; j < _EFI_PALETTE_COLORS_MAX; j++) {
                        if (j > 0)
                                printf(", ");
                        size_t color = config->efi_palletes[i][j];
...
static void config_defaults_load_from_file(Config *config, char *content) {
        ...
        while ((line = line_get_key_value(content, " \t", &pos, &key, &value)))
                if (streq8(key, "timeout")) {
                        ...
                        if (palette_type < 0)
                                continue;
                        parse_palette(palette_type, value, &config->palettes[palette_type]);
                }
}
